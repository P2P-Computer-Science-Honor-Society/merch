...
class Printful_Account():
    ...
    def get_printfiles(self, id):
      url = self.base_url + "/mockup-generator/printfiles/%s" %id
      return self.do_get(url)

    def get_mockup_task(self, task_key):
      url = self.base_url + "/mockup-generator/task"
      return requests.get(url, params={"task_key": task_key}, headers=self.headers)
...
rjson = response.json()
task_key = rjson['result']['task_key']
time.sleep(5)
r = printful.get_mockup_task(task_key)
print(r)
print(r.json())
